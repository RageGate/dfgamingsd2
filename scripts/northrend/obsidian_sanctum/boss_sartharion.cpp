...
struct MANGOS_DLL_DECL boss_sartharionAI : public ScriptedAI
{
    ...
    void CallDragon(uint32 uiDataId)
    {
        if (m_pInstance)
        {
            ...
            if (pTemp && pTemp->isAlive() && !pTemp->getVictim())
            {
                ...
                DoCastSpellIfCan(m_creature, SPELL_WILL_OF_SARTHARION, false);
                bCanUseWill = true;
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if (!m_bIsHardEnraged)
        {
            ...
            {
                DoCastSpellIfCan(m_creature, SPELL_PYROBUFFET, true);
...
struct MANGOS_DLL_DECL mob_fire_cycloneAI : public ScriptedAI
{
    ...
    void SpellHit(Unit* pCaster, const SpellEntry* pSpell)
    {
        ...
        {
            DoCastSpellIfCan(m_creature, 57560, true);
    ...
    void UpdateAI(const uint32 uiDiff)
    {
      if(m_pInstance && m_pInstance->GetData(TYPE_SARTHARION_EVENT) != IN_PROGRESS) return;
        ...
        if (m_bLavaStrikeAllowed)
        {
            if (m_uiLavaStrikeTimer < uiDiff)
            {
                if (Creature* pTemp = m_creature->SummonCreature(25640, urand(3220, 3275), urand(486, 575), 58.8f, 0, TEMPSUMMON_TIMED_DESPAWN, 15000))
                {
                    ...
                    DoCastSpellIfCan(pTemp, SPELL_LAVA_STRIKE);
                }
        ...
        if (!m_bLavaStrikeAllowed)
        {
            ...
            {
               DoCastSpellIfCan(m_creature, SPELL_CYCLONE_AURA_2);
...
struct MANGOS_DLL_DECL mob_flame_tsunamiAI : public ScriptedAI
{
    ...
    void Reset()
    {
        ...
        DoCastSpellIfCan(m_creature, SPELL_FLAME_TSUNAMI, true);
