...
enum
{
    ...
    //Not much choice about these. We have to make own definition on the direction/start-end point
    ...
    SPELL_SUMMON_GUARD          = 68968,

    NPC_WHELP                   = 11262,
    NPC_LAIR_GUARD              = 36561,
...
static float afSpawnLocations[4][3]=
{
    ...
    {-85.460f, -214.090f, -82.507f},
    {-22.347f, -214.571f, -89.104f},
};
...
struct boss_onyxiaAI : public ScriptedAI
{
    ...
    void Reset()
    {
        ...
        m_uiMovePoint = urand(0, 5);//7);
        m_uiMovementTimer = 20000;
        m_pPointData = GetMoveData();

        m_uiFireballTimer = 15000;
        m_uiSummonWhelpsTimer = 10000;
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if (m_uiPhase == PHASE_START || m_uiPhase == PHASE_END)
        {
            ...
            {
                if (DoCastSpellIfCan(m_creature, m_bIsRegularMode ? SPELL_FLAMEBREATH : H_SPELL_FLAMEBREATH) == CAST_OK)
                    m_uiFlameBreathTimer = urand(10000, 20000);
            }
            else
            ...
            {
                if (DoCastSpellIfCan(m_creature, m_bIsRegularMode ? SPELL_TAILSWEEP : H_SPELL_TAILSWEEP) == CAST_OK)
                    m_uiTailSweepTimer = urand(15000, 20000);
            }
            else
            ...
            {
                if (DoCastSpellIfCan(m_creature, m_bIsRegularMode ? SPELL_WINGBUFFET : H_SPELL_WINGBUFFET) == CAST_OK)
            ...
            if (m_creature->GetDistance2d(-22.34f, -214.57f) > 70.0f)
            {
                EnterEvadeMode();
                return;
            }
        ...
        else
        {
            ...
            if (m_uiFireballTimer < uiDiff)
            {
                if (m_creature->GetMotionMaster()->GetCurrentMovementGeneratorType() != POINT_MOTION_TYPE)
                {
                    ...
                    {
                        if (DoCastSpellIfCan(pTarget, m_bIsRegularMode ? SPELL_FIREBALL : H_SPELL_FIREBALL) == CAST_OK)
            ...
            {
                m_creature->SummonCreature(NPC_LAIR_GUARD, afSpawnLocations[2][0], afSpawnLocations[2][1], afSpawnLocations[2][2], 0.0f, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 30000);
                m_uiSummonGuardTimer = 30000;
            }
            else
        ...
        if (m_uiPhase == PHASE_BREATH || m_uiPhase == PHASE_END)
        {
